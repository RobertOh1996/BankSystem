...
/**
 * Business savings account dao implementation.
 * 
...
public class BusinessSavingsAccountDAOImpl implements AccountDAO {

	/*
	 * The database to connect to.
	 */
	private static final String DATABASE_NAME = "businesssavingsaccounts.txt";
	 ...
	 */
	public BusinessSavingsAccountDAOImpl(AccountTableDAO accountTableDAO, TransactionDAO transactionDAO) {
	 ...
	 * @param accountTableDAO: Provides access to the accounts a user has.
	 */
	public BusinessSavingsAccountDAOImpl(AccountTableDAO accountTableDAOImpl) {
	...
	public Account getAccount(String accountNumber) throws FileNotFoundException {
		...
		while (accountScanner.hasNextLine()) {
			...
			if (accountNumber.equals(fileAccountNumber)) {
				account = new BusinessSavingsAccount(accountLine[0], AccountType.valueOf(accountLine[1]),
						new BigDecimal(accountLine[2]), this, transactionDAO);
			}
		}
		if (account == null) {
			accountScanner.close();
			throw new IllegalArgumentException("Business Savings Account " + accountNumber + " does not exist.");
		}
		accountScanner.close();
		return account;
	}
	...
	Account createAccount() throws IOException {
		...
		Account account = new BusinessSavingsAccount(String.valueOf(nextAccountNumber));
		return account;
	}
}

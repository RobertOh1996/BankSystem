...
/**
 * Business checking account dao implementation.
 * 
...
public class BusinessCheckingAccountDAOImpl implements AccountDAO {

	/*
	 * The database to connect to.
	 */
	private static final String DATABASE_NAME = "businesscheckingaccounts.txt";
	 ...
	 */
	public BusinessCheckingAccountDAOImpl(AccountTableDAO accountTableDAO, TransactionDAO transactionDAO) {
	 ...
	 * @param accountTableDAO: Provides access to the accounts a user has.
	 */
	public BusinessCheckingAccountDAOImpl(AccountTableDAO accountTableDAOImpl) {
	...
	public Account getAccount(String accountNumber) throws FileNotFoundException {
		...
		while (accountScanner.hasNextLine()) {
			...
			if (accountNumber.equals(fileAccountNumber)) {
				account = new BusinessCheckingAccount(accountLine[0], AccountType.valueOf(accountLine[1]),
						new BigDecimal(accountLine[2]), Boolean.valueOf(accountLine[3]), this, transactionDAO);
			}
		}
		if (account == null) {
			accountScanner.close();
			throw new IllegalArgumentException("Business Checking Account " + accountNumber + " does not exist.");
		}
		accountScanner.close();
		return account;
	}
	...
	Account createAccount() throws IOException {
		...
		Account account = new BusinessCheckingAccount(String.valueOf(nextAccountNumber));
		return account;
	}
